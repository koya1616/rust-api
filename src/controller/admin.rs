...
pub fn login(login: Form<AdminLoginRequestBody>) -> Result<Json<AdminLoginResponseBody>, Custom<Json<ErrorResponse>>> {
  admin_users
    .filter(email.eq(&login.email))
    .first::<AdminUser>(&mut establish_connection())
    .map_err(|error| match error {
      diesel::result::Error::NotFound => create_error(Status::NotFound, "ユーザーが見つかりません"),
